...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Step 5: Fetch details with no timeout (edge runtime)
    async function fetchBatch(items, urlFn, extractFn) {
      const results = [];
      for (let i = 0; i < items.length; i += 5) {
        const batch = items.slice(i, i + 5);
        const batchRes = await Promise.all(
          batch.map(item => fetch(urlFn(item), { headers: zH })
            .then(r => r.json()).then(d => extractFn(d)).catch(() => null))
        );
    ...
    const [soDetails, invDetails] = await Promise.all([
      fetchBatch(confirmedSOs, so => `${BASE}/salesorders/${so.salesorder_id}?organization_id=${ORG}`, d => d.salesorder),
      fetchBatch(bfld, inv => `${BASE}/invoices/${inv.invoice_id}?organization_id=${ORG}`, d => d.invoice)
    ]);

    // Step 6: Process line items with real SOH + ETA + FAP logic
    function processLineItems(lineItems, orderESD, orderRef) {
      if (!lineItems?.length) return [];
      const esdDays = days(orderESD);
      return lineItems
        .filter(li => li.item_type === 'inventory')
        .map(li => {
          const sku = String(li.sku || '').trim();
          const name = li.name || '';
          const qty = parseFloat(li.quantity) || 1;
          const soh = parseFloat(li.stock_on_hand) || 0;
          const eta = lookupETA(sku, etaMap);
          const etaDays = days(eta);

          let status, statusLabel, detail;

          if (soh >= qty) {
            // SOH covers the order quantity
            status = 'stock';
            statusLabel = `In stock (SOH: ${soh})`;
            detail = '';
          } else if (eta) {
            if (etaDays !== null && esdDays !== null && etaDays <= esdDays) {
              // Stock arrives before or on ESD — fulfillable via ETA
              status = 'eta';
              statusLabel = `ETA ${eta}`;
              detail = `Stock arriving ${eta} — before order ESD ${orderESD}`;
            } else {
              // Stock arrives AFTER ESD — need FAP
              // Find other BFLD orders that have this SKU booked, with low advance and ESD after ETA
              const fapCandidates = bfld.filter(inv => {
                if (inv.invoice_number === orderRef) return false;
                const t = parseFloat(inv.total)||0, b = parseFloat(inv.balance)||0;
                const a = t > 0 ? Math.round(((t-b)/t)*100) : 0;
                const cf = inv.custom_field_hash || {};
                const invEsd = cf.cf_expected_shipment_date_unformatted || inv.due_date || '';
                const invEsdDays = days(invEsd);
                // FAP candidate: low advance AND their ESD is after the ETA (they can wait)
                return a < 30 && invEsdDays !== null && etaDays !== null && invEsdDays > etaDays;
              }).slice(0, 3);

              if (fapCandidates.length > 0) {
                status = 'fap';
                statusLabel = 'FAP possible';
                detail = `ETA ${eta} > order ESD ${orderESD}. Can reallocate from: ` +
                  fapCandidates.map(c => {
                    const t=parseFloat(c.total)||0, b=parseFloat(c.balance)||0;
                    const a=t>0?Math.round(((t-b)/t)*100):0;
                    const cf = c.custom_field_hash || {};
                    const cEsd = cf.cf_expected_shipment_date_unformatted || c.due_date || '';
                    return `${c.invoice_number} (${c.customer_name}, ${a}% adv, ESD ${cEsd})`;
                  }).join(' | ');
              } else {
                status = 'eta';
                statusLabel = `ETA ${eta} (after ESD)`;
                detail = `ETA ${eta} is after order ESD ${orderESD} — no suitable FAP candidate found`;
              }
            }
          } else {
            // Not in ETA sheet — no stock, no incoming shipment
            status = 'blocked';
            statusLabel = 'Not in ETA sheet';
            detail = `SKU ${sku} has no incoming stock — raise import/purchase order`;
          }
    ...
    function classify(ref, cust, sp, esd, confirmed, total, bal, note, sub, id, type, lineItems) {
      ...
      const lines = processLineItems(lineItems || [], esd, ref);
      ...
      const allStock = lines.length > 0 && lines.every(l => l.status === 'stock');

      let verdict = 'fulfillable', vl = 'Ready', action = 'Book shipping';
      if (sub?.includes('Cancelled')) { verdict='urgent'; vl='Cancelled'; action='Refund or dispatch?'; }
      else if (d !== null && d < 0) { verdict='urgent'; vl=`${Math.abs(d)}d lapsed`; action='Contact customer'; }
      ...
      else if (allStock) { verdict='fulfillable'; vl='Ready'; action='Book shipping now'; }
