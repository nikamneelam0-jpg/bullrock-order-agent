  ...
  try {
    ...
    try {
      ...
      rows.forEach(row => {
        ...
        if (sku && eta) {
          etaMap[sku] = eta;
          etaMap[sku.replace(/^0+/,'')] = eta;
          etaMap[sku.padStart(4,'0')] = eta;
          etaMap[sku.padStart(5,'0')] = eta;
        }
      });
    ...
    function getLineItems(lineItems, orderESD) {
      ...
      return lineItems
        ...
        .map(li => {
          ...
          // Try multiple SKU formats for matching
        const skuClean = sku.replace(/^0+/,'');
        const eta = etaMap[sku] || etaMap[skuClean] || 
                    etaMap[sku.padStart(4,'0')] || etaMap[sku.padStart(5,'0')] || 
                    etaMap[sku.padStart(6,'0')] || null;
